...
The Notiky [Model Context Protocol](https://modelcontextprotocol.io) surface lets coding agents (Cursor, Claude Code, Codex) read and write your workspace brain — decisions, lessons, context packs, and project state — over a standard MCP transport.
...
```bash
notiky mcp tokens create [name]    # Create a token (prints the secret once)
notiky mcp tokens list             # List tokens for the current workspace
notiky mcp tokens revoke <id>      # Revoke a token by ID
```
...
```bash
notiky mcp serve --workspace acme
notiky mcp serve --workspace 7c5e... --transport stdio
```
...
```bash
notiky mcp call <tool> --arg key=value [--arg key=value ...] [--token <token>]
```

Use `--token` or set `NOTIKY_MCP_TOKEN`. Examples:

```bash
notiky mcp call notiky.profile --arg project_id=<uuid>
notiky mcp call notiky.search_lessons --arg project_id=<uuid> --arg query="materialized views"
```
...
```bash
notiky resolve project                       # From $PWD's git origin remote
notiky resolve project --path /path/to/repo  # From an explicit directory
```

Prints just the project UUID on success. Capture it for downstream commands:

```bash
PROJECT_ID=$(notiky resolve project --path "$PWD")
notiky mcp call notiky.profile --arg project_id="$PROJECT_ID"
```
